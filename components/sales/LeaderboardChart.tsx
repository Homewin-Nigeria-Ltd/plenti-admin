...
export default function LeaderboardChart({ data }: LeaderboardChartProps) {
  console.log("LeaderboardChart data:", data);
  const hasData = Array.isArray(data) && data.length > 0;
  const topSalesRep = hasData ? data[0] : null;

  const chartData = hasData
    ? data.map((entry) => ({
        name: entry.name,
        value: entry.total_achieved,
      }))
    : [];

  return (
    <div className="rounded-lg border border-[#EFEFEF] bg-white p-6 h-fit!">
        ...
        <div>
          ...
          <p className="text-[33px] font-bold text-[#2B3674] leading-tight">
            {topSalesRep ? (
              ...
              <span className="text-[#A3AED0]">No data</span>
            )}
          </p>
        </div>
        {/* {hasData && (
