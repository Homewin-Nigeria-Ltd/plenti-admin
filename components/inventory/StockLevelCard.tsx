...
const StockLevelChart = dynamic(() => import("./StockLevelChart"), {
  ssr: false,
  loading: () => (
    <div className="w-[300px] h-[300px] bg-[#EEF1F6] rounded-full animate-pulse" />
  ),
});
...
export default function StockLevelCard({ stockLevel }: StockLevelCardProps) {
  return (
    <div className="bg-white rounded-xl border border-[#EAECF0] p-4 sm:p-6 shadow-xs">
      <h3 className="text-primary-700 font-semibold text-sm sm:text-base mb-4 sm:mb-6">
        Stock Level
      </h3>
        ...
        <div className="flex-1 flex flex-col justify-between">
          <div className="flex flex-col gap-2">
            ...
            <span className="text-xs sm:text-sm font-medium text-neutral-700">
              {new Intl.NumberFormat("en-US").format(stockLevel.highStockItems)}{" "}
              Items
            </span>
          </div>
          ...
          <div className="flex flex-col gap-2">
            ...
            <span className="text-xs sm:text-sm font-medium text-neutral-700">
              {new Intl.NumberFormat("en-US").format(stockLevel.lowStockItems)}{" "}
              Items
            </span>
          </div>
        </div>
