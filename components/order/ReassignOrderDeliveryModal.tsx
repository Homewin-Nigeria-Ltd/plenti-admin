...
import { ORDERS_API, orderAssignmentReassignPath } from "@/data/orders";
...
type ReassignOrderDeliveryModalProps = {
  isOpen: boolean;
  onClose: () => void;
...
export function ReassignOrderDeliveryModal({
  isOpen,
  onClose,
  deliveryId,
  orderNumber,
  currentRiderId,
  onSuccess,
}: ReassignOrderDeliveryModalProps) {
  ...
  const fetchRiders = React.useCallback(async () => {
    ...
    try {
      ...
      if (data?.status === "success" && Array.isArray(data?.data)) {
        setRiders(
          data.data.filter(
            (rider) => currentRiderId == null || rider.id !== currentRiderId,
          ),
        );
      } else {
        setRiders([]);
      ...
      setRiders([]);
      toast.error(
        getApiErrorMessage(error) ?? "Failed to load riders",
      );
    } finally {
  ...
  const handleReassign = async () => {
    ...
    if (deliveryId == null) {
      toast.error("This order has no Plenti delivery to reassign");
      return;
    }
    ...
    try {
      const { data } = await api.post<{ status?: string; message?: string }>(
        orderAssignmentReassignPath(deliveryId),
        { new_rider_id: newRiderId },
      );

      if (data?.status === "error" || data?.status === "failed") {
        toast.error(data?.message ?? "Failed to reassign rider");
        return;
      }
      ...
      toast.error(getApiErrorMessage(error) ?? "Failed to reassign rider");
    } finally {
        ...
        <DialogHeader className="relative px-6 pt-6 pb-4 border-b border-neutral-100">
          ...
          <DialogDescription className="text-sm text-[#667085]">
            {orderNumber
              ...
              : "Choose a new rider for this delivery"}
          </DialogDescription>
          <button
            type="button"
            onClick={handleClose}
